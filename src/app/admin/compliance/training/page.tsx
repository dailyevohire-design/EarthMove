...
export default async function Page() {
  ...
  catch (e) { if (e instanceof UnauthorizedError) redirect('/login?next=/admin/compliance/training'); if (e instanceof ForbiddenError) redirect('/'); throw e; }
  const sb = createComplianceClient();
  const { data } = await sb.from('training_log').select('*').limit(200);
  const rows = data ?? [];

  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      <p className="text-xs uppercase tracking-widest text-emerald-700"><Link href="/admin/compliance" className="hover:underline">Compliance</Link> · training</p>
      <h1 className="mt-2 text-2xl font-semibold">Training log</h1>
      <p className="mt-1 text-sm text-stone-500">{rows.length} rows · table <code className="font-mono text-xs">compliance.training_log</code></p>
