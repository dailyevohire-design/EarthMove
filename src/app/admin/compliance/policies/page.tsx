...
export default async function Page() {
  ...
  catch (e) { if (e instanceof UnauthorizedError) redirect('/login?next=/admin/compliance/policies'); if (e instanceof ForbiddenError) redirect('/'); throw e; }
  const sb = createComplianceClient();
  const table = TABLE_MAP['policies'];
  const { data } = await sb.from(table).select('*').limit(200);
  const rows = data ?? [];

  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      <p className="text-xs uppercase tracking-widest text-emerald-700"><Link href="/admin/compliance" className="hover:underline">Compliance</Link> · policies</p>
      <h1 className="mt-2 text-2xl font-semibold">{('policies').replace(/-/g, ' ').replace(/^\w/, (c) => c.toUpperCase())}</h1>
