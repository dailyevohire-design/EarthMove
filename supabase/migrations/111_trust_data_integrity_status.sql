-- Migration 106: data integrity status for trust_reports + lazy re-pull foundation
-- Why: 159 of 167 evidence-bearing reports have dangling evidence_ids (UUIDs
-- pointing to non-existent trust_evidence rows). They cannot be projected from
-- existing data. Add a flag so the contractor UI can render a "refresh report"
-- CTA instead of empty boxes, and so admin queries can bulk-replay later.
--
-- Status values:
--   ok      - every evidence_id resolves to a real evidence row; safe to render
--   stale   - some/all evidence_ids dangling, OR no evidence + no raw_report
--   legacy  - no evidence_ids but raw_report populated (v1 sync free/pro path)
--   unknown - default before backfill (should not persist after this migration)

ALTER TABLE trust_reports
  ADD COLUMN IF NOT EXISTS data_integrity_status text NOT NULL DEFAULT 'unknown';
...
ALTER TABLE trust_reports
  ...
  CHECK (data_integrity_status IN ('ok','stale','legacy','unknown'));

CREATE INDEX IF NOT EXISTS idx_tr_integrity_warn
  ON trust_reports(data_integrity_status, created_at DESC)
  WHERE data_integrity_status IN ('stale','legacy');

-- Fold integrity computation into the projection function (single UPDATE,
-- no extra round-trip). Function signature unchanged.
...
-- Backfill: one-shot status compute, no re-projection (faster than calling the fn)
UPDATE trust_reports SET data_integrity_status = CASE
  WHEN cardinality(evidence_ids) = 0 THEN
    CASE WHEN raw_report IS NOT NULL AND raw_report <> '{}'::jsonb THEN 'legacy' ELSE 'stale' END
  WHEN (SELECT count(*) FROM trust_evidence WHERE id = ANY(evidence_ids)) = cardinality(evidence_ids)
    THEN 'ok'
  ELSE 'stale'
END;

COMMENT ON COLUMN trust_reports.data_integrity_status IS
'ok = all evidence resolves; stale = dangling pointers or empty; legacy = v1 path with raw_report only. UI renders refresh CTA when != ok.';
