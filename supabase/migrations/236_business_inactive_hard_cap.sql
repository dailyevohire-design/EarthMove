-- 236_business_inactive_hard_cap.sql
--
-- Adds business_inactive hard cap to calculate_contractor_trust_score.
-- Mirrors business_dissolved (cap 20) and business_not_found (cap 35)
-- precedent. Inactive CO/TX LLCs legally cannot transact, same operational
-- effect as dissolved — without this cap, an inactive contractor scored
-- 62/MEDIUM (Bedrock excavating example after mig 235) instead of the
-- correct CRITICAL band.
--
-- Function is CREATE OR REPLACE — signature preserved, only the body
-- changes (new v_business_inactive flag + cap clause + inputs_snapshot
-- key).
--
-- After the function swap, re-scores all trust_reports whose job has
-- business_inactive evidence: UPDATE trust_reports.{trust_score,
-- risk_level, score_breakdown} + UPDATE contractor_trust_scores in place
-- so the latest snapshot reflects the new cap.
...
DECLARE
  ...
  v_business_inactive       BOOLEAN := FALSE;
...
BEGIN
  ...
  -- 236: new flag — inactive entities legally cannot transact, same product effect as dissolved
  SELECT EXISTS(SELECT 1 FROM trust_evidence WHERE job_id = p_job_id AND finding_type = 'business_inactive')
    INTO v_business_inactive;
  ...
  ELSE
    ...
    END IF;
    -- 236: business_inactive cap. Inactive entities legally cannot transact;
    -- mirrors business_not_found (cap 35) severity.
    IF v_business_inactive AND v_composite > 35 THEN
      v_composite := 35;
      v_caps_applied := v_caps_applied || jsonb_build_object('cap','business_inactive','value',35);
    END IF;
  ...
  v_inputs := jsonb_build_object(
    ...
    'business_inactive', v_business_inactive,
...
-- Re-score all reports whose job has business_inactive evidence. UPDATE
-- trust_reports and contractor_trust_scores in place — same row, new
-- composite reflecting the cap.
DO $$
DECLARE
  r RECORD;
...
BEGIN
  FOR r IN
    SELECT DISTINCT tr.id AS report_id, tr.job_id
    FROM trust_reports tr
    WHERE tr.job_id IS NOT NULL
      AND EXISTS (
        SELECT 1 FROM trust_evidence te
        WHERE te.job_id = tr.job_id AND te.finding_type = 'business_inactive'
      )
  LOOP
    v_score := calculate_contractor_trust_score(r.job_id);
    ...
    SET composite_score       = v_score.composite_score,
        grade                 = v_score.grade,
        risk_level            = v_score.risk_level,
        license_score         = v_score.license_score,
        business_entity_score = v_score.business_entity_score,
        legal_score           = v_score.legal_score,
        osha_score            = v_score.osha_score,
        bbb_score             = v_score.bbb_score,
        phoenix_score         = v_score.phoenix_score,
        age_score             = v_score.age_score,
        effective_weights     = v_score.effective_weights,
        evidence_count        = v_score.evidence_count,
        structured_hit_rate   = v_score.structured_hit_rate,
        inputs_snapshot       = v_score.inputs_snapshot,
        calculated_at         = NOW()
