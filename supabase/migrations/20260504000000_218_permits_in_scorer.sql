-- Migration 218: add permits as 8th category in calculate_contractor_trust_score
-- Permit history is already extracted as evidence (1,165 rows in prod) but ignored
-- by the deterministic scorer. This adds permit_score with weight 0.05.
-- The existing denominator-normalization pattern handles the new category cleanly:
-- missing permit signal -> weight zeros out -> no impact on contractors without permits.
-- contractor_trust_scores table has NO permit_score column; permit info lives in
-- inputs_snapshot jsonb only. ROW constructor stays at 23 fields.
...
BEGIN
  ...
  -- PERMITS (NEW - migration 218)
  -- Pick strongest signal: robust > clean > low > stale (descending positivity).
  -- Stale ranks last because 'had permits, doesn't anymore' is the credibility flag.
