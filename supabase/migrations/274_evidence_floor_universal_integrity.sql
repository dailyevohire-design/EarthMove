-- Migration 274: Extend evidence-floor trigger to ALL score bands.
-- Universal write-boundary rule: any non-null trust_score requires
-- data_integrity_status='ok'. CRITICAL band preserves the mig 272
-- adverse-evidence check. Supersedes mig 272 CRITICAL-only logic.
-- (Final form is in mig 275 — this is the intermediate step kept for parity.)
...
BEGIN
  ...
  IF NEW.trust_score < 40 THEN
    ...
    IF COALESCE(v_adverse_count, 0) < 1 THEN
      NEW.trust_score := NULL; NEW.risk_level := NULL; NEW.biz_status := NULL;
      NEW.requires_re_review := TRUE;
      RAISE WARNING 'report % CRITICAL score blocked — zero adverse evidence for contractor %', NEW.id, NEW.contractor_id;
    END IF;
  END IF;
  RETURN NEW;
END;
...
COMMENT ON FUNCTION public.enforce_trust_score_evidence_floor() IS
  'Universal write-boundary evidence floor (mig 274). Superseded by mig 275 which extends adverse-evidence requirement to HIGH band.';
