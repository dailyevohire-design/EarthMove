-- Migration 221: expand license_score vocabulary in calculate_contractor_trust_score
--
-- Background: 2026-05-04 data piece run found license_score returned NULL on
-- 92 of 93 entities. Root cause: scrapers co_dora and tx_tdlr emit finding_types
-- (license_no_record, license_revoked, license_disciplinary_action) that the
-- scorer's CASE statement doesn't recognize. NULL falls through and license_score
-- weight gets zeroed, contributing to the bimodal 53/97 distribution.
--
-- This migration extends the scorer's CASE to recognize all three new vocab
-- entries with sane scores:
--   license_revoked              -> 0   (catastrophic; outranks suspended)
--   license_disciplinary_action  -> 30  (active license but with discipline on record)
--   license_no_record            -> 75  (discipline scrape found nothing — positive but not as strong as confirmed license_active)
--
-- Function body is otherwise identical to migration 218 (permits_in_scorer).
-- Only the LICENSING block changed (CASE values + ORDER BY priorities).
...
BEGIN
  ...
  -- LICENSING (vocab expanded — see migration 221 header for context)
  ...
  -- BUSINESS (unchanged - migration 127 priority swap)
  SELECT CASE finding_type
  ...
  -- WEIGHTS - zero out missing categories
...
COMMENT ON FUNCTION public.calculate_contractor_trust_score(uuid) IS
  'v3 (migration 221): adds license_no_record (75), license_revoked (0), license_disciplinary_action (30) to license_score CASE so co_dora and tx_tdlr discipline-source findings score correctly instead of falling through to NULL. Other categories unchanged from migration 218.';
